package chooongg.libAndroid.core.widget.ratio
...
class RatioConstraintLayout @JvmOverloads constructor(
    ...
    init {
        ...
        val sizeRatio: String? = array.getString(R.styleable.RatioConstraintLayout_sizeRatio)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (widthRatio != 0f && heightRatio != 0f) {
            ...
            if ((layoutParams.width != FrameLayout.LayoutParams.WRAP_CONTENT) && (layoutParams.height != FrameLayout.LayoutParams.WRAP_CONTENT) &&
            ...
            } else if ((layoutParams.width != FrameLayout.LayoutParams.WRAP_CONTENT) && (widthSpecMode == MeasureSpec.EXACTLY) && (heightSpecMode != MeasureSpec.EXACTLY)) {
                // 如果当前宽度是写死的，但是高度不写死
                finalHeightMeasureSpec = MeasureSpec.makeMeasureSpec(
                    (widthSpecSize / sizeRatio).toInt(),
                    MeasureSpec.EXACTLY
                )
            } else if ((layoutParams.height != FrameLayout.LayoutParams.WRAP_CONTENT) && (heightSpecMode == MeasureSpec.EXACTLY) && (widthSpecMode != MeasureSpec.EXACTLY)) {
                // 如果当前高度是写死的，但是宽度不写死
                finalWidthMeasureSpec = MeasureSpec.makeMeasureSpec(
                    (heightSpecSize * sizeRatio).toInt(),
                    MeasureSpec.EXACTLY
