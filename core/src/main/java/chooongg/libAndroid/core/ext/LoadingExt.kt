...
import chooongg.libAndroid.basic.ext.gone
import chooongg.libAndroid.basic.ext.launchMain
import chooongg.libAndroid.basic.ext.resString
import chooongg.libAndroid.basic.ext.visible
...
fun Activity.showLoading(@StringRes resId: Int, vararg format: Any?) {
    if (format.isEmpty()) {
        showLoading(resString(resId))
    } else {
        showLoading(resString(resId, *format))
    }
}
...
private fun Activity.showLoadingForActivity(message: CharSequence?) {
    var loadingView = window.decorView.findViewById<View>(R.id.box_activity_loading)
    if (loadingView != null) {
        loadingView.animate().setListener(null).cancel()
    } else {
        loadingView =
            LayoutInflater.from(this).inflate(R.layout.lib_activity_loading, null)
...
private fun Activity.hideLoadingForActivity() {
    window.decorView.findViewById<View>(R.id.box_activity_loading)?.apply {
        animate().setInterpolator(AccelerateDecelerateInterpolator())
            ...
            .setListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    (window.decorView as ViewGroup).removeView(this@apply)
                }
            })
    }
}